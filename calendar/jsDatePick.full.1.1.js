...
JsDatePick = function(configurationObject){
	...
	this.monthsTextualRepresentation = new Array("Janaury","February","March","April","May","June","July","August","September","October","November","December");
...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	this.oConfiguration.yearsRange		= (aConf["yearsRange"] != null) ? aConf["yearsRange"] : new Array(1971,2100);
	...
	if (this.oConfiguration.selectedDate != null){
		...
		if (typeof(this.oConfiguration.selectedDate) == "object"){
			if (!isNaN(this.oConfiguration.selectedDate.day) &&
				...
				this.currentMonth 	= this.selectedDayObject.month 	= this.oConfiguration.selectedDate.month;
				this.currentYear 	= this.selectedDayObject.year	= this.oConfiguration.selectedDate.year;
...
JsDatePick.prototype.populateFieldWithSelectedDate = function(){
	$(this.oConfiguration.target).value = this.getSelectedDayFormatted();
	this.closeCalendar();
};
...
JsDatePick.prototype.makeCalendar = function(){
	...
	JsDatePickBox.setAttribute("group","JsDatePickBox");
	clearfix.setAttribute("group","clearfix");
	closeButton.setAttribute("group","jsDatePickCloseButton");
	closeButton.setAttribute("globalNumber",this.globalNumber);
	
	closeButton.onmouseover = function(){
		this.setAttribute("group","jsDatePickCloseButtonOver");
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Close the calendar\");");
	};
	
	closeButton.onmouseout = function(){
		this.setAttribute("group","jsDatePickCloseButton");
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"\");");
	};
	
	closeButton.onmousedown = function(){
		this.setAttribute("group","jsDatePickCloseButtonDown");
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Close the calendar\");");
	};
	...
	closeButton.onmouseup = function(){
		this.setAttribute("group","jsDatePickCloseButton");
		var globalRef = "gRef_"+this.getAttribute("globalNumber");
		eval("gRef_"+this.getAttribute("globalNumber")+".closeCalendar();");
		eval(globalRef + ".setTooltipText(\"\");");
	};
	...
	topWall.setAttribute("group","topWall");
	bottomWall.setAttribute("group","bottomWall");
	
	var topCorner	 = d.createElement("div");
	var bottomCorner = d.createElement("div");
	var wall		 = d.createElement("div");
	
	topCorner.setAttribute("group","leftTopCorner");
	bottomCorner.setAttribute("group","leftBottomCorner");
	wall.setAttribute("group","leftWall");
	...
	topCorner.setAttribute("group","rightTopCorner");
	bottomCorner.setAttribute("group","rightBottomCorner");
	wall.setAttribute("group","rightWall");
	...
	if (this.oConfiguration.isStripped){
		leftWall.setAttribute("group","hiddenBoxLeftWall");
		rightWall.setAttribute("group","hiddenBoxRightWall");
	} else {
		leftWall.setAttribute("group","boxLeftWall");
		rightWall.setAttribute("group","boxRightWall");
	}
	...
	if (this.oConfiguration.useMode == 2){
		if (this.oConfiguration.target != false){
			if (typeof($(this.oConfiguration.target)) != null){
				var inputElement = $(this.oConfiguration.target);
				...
				var aFunc = new Function("gRef_"+this.globalNumber+".populateFieldWithSelectedDate();");
		...
		if (this.oConfiguration.target != null){
			...
			$(this.oConfiguration.target).style.position = "relative";
			JsDatePickBox.style.position = "absolute";
...
JsDatePick.prototype.getDOMCalendarStripped = function(){
	...
	var weekDaysRow				= d.createElement("div");
	var clearfix2				= d.createElement("div");
	
	clearfix.setAttribute("group","clearfix");
	clearfix2.setAttribute("group","clearfix");
	boxMainInner.setAttribute("group","boxMainInner");
	boxMainCellsContainer.setAttribute("group","boxMainCellsContainer");
	tooltip.setAttribute("group","tooltip");
	weekDaysRow.setAttribute("group","weekDaysRow");
...
JsDatePick.prototype.makeDOMWeekDays = function(aWeekDaysRow){
	var i=0;
	var d = document;
	var weekDaysArray = new Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");
	var textNode;
	var weekDay;
	...
	for (i=0; i<7; i++){
		...
		weekDay.setAttribute("group","weekDay");

		if (weekDaysArray[i] == "Sat"){
...
JsDatePick.prototype.populateMainBox = function(aMainBox){
	...
	var aTextNode;
	var columnNumber = 1;
	var disabledDayFlag = false;
	
	var cmpMonth = this.currentMonth-1;
	
	var oDay 	= new Date(this.currentYear, cmpMonth, 1,1,0,0);
	var iStamp	= oDay.getTime();
	
	this.flag_DayMarkedBeforeRepopulation = false;
	this.setControlBarText(this.monthsTextualRepresentation[cmpMonth] + ", " + this.currentYear);
	
	var skipDays = parseInt(oDay.getDay());
	var i=0;
	for (i=0; i<skipDays; i++){
		aDayDiv = d.createElement("div");
		aDayDiv.setAttribute("group","skipDay");
	...
	while (oDay.getMonth() == cmpMonth){
		...
		aDayDiv.onmouseover = function(){
			...
			var globalRef = "gRef_"+this.getAttribute("globalNumber");
			var currentColorScheme = eval(globalRef + ".getCurrentColorScheme()");
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.setAttribute("group","dayOverToday");
				this.style.background = "url(img/" + currentColorScheme + "_dayOver.gif) left top no-repeat";
			} else {
				this.setAttribute("group","dayOver");
				this.style.background = "url(img/" + currentColorScheme + "_dayOver.gif) left top no-repeat";
			}
		};
		...
		aDayDiv.onmouseout = function(){
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.setAttribute("group","dayNormalToday");
				this.style.background = "url(img/" + currentColorScheme + "_dayNormal.gif) left top no-repeat";
			} else {
				this.setAttribute("group","dayNormal");
				this.style.background = "url(img/" + currentColorScheme + "_dayNormal.gif) left top no-repeat";
			}
		};
		...
		aDayDiv.onmousedown = function(){
			...
			if (parseInt(this.getAttribute("isToday")) == 1){
				this.setAttribute("group","dayDownToday");
				this.style.background = "url(img/" + currentColorScheme + "_dayDown.gif) left top no-repeat";
			} else {
				this.setAttribute("group","dayDown");
				this.style.background = "url(img/" + currentColorScheme + "_dayDown.gif) left top no-repeat";
			}
		};
		...
		if (this.isSelectedDay(oDay.getDate())){
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				aDayDiv.setAttribute("group","dayDownToday");
				aDayDiv.style.background = "url(img/" + this.oConfiguration.cellColorScheme + "_dayDown.gif) left top no-repeat";
			} else {
				aDayDiv.setAttribute("group","dayDown");
				aDayDiv.style.background = "url(img/" + this.oConfiguration.cellColorScheme + "_dayDown.gif) left top no-repeat";
			}
			
		} else {
			
			var globalRef = "gRef_"+aDayDiv.getAttribute("globalNumber");
			var currentColorScheme = eval(globalRef + ".getCurrentColorScheme()");
			
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				if (disabledDayFlag){
					aDayDiv.setAttribute("group","dayDisabled");
					aDayDiv.style.background = "url(img/" + this.oConfiguration.cellColorScheme + "_dayNormal.gif) left top no-repeat";
				} else {
					aDayDiv.setAttribute("group","dayNormalToday");
					aDayDiv.style.background = "url(img/" + this.oConfiguration.cellColorScheme + "_dayNormal.gif) left top no-repeat";
				}
			} else {
				if (disabledDayFlag){
					...
					aDayDiv.setAttribute("group","dayNormal");
					aDayDiv.style.background = "url(img/" + this.oConfiguration.cellColorScheme + "_dayNormal.gif) left top no-repeat";
				}
			}
		}
...
JsDatePick.prototype.getSelectedDayFormatted = function(){
	if (this.flag_aDayWasSelected){
		...
		var returnVal = this.selectedDayObject.year+"-";
		
		if (this.selectedDayObject.month >= 1 && this.selectedDayObject.month <= 9){
...
JsDatePick.prototype.setDaySelection = function(anElement){
	...
	var globalRef = "gRef_"+anElement.getAttribute("globalNumber");
	var currentColorScheme = eval(globalRef + ".getCurrentColorScheme()");
	...
	if  (this.flag_DayMarkedBeforeRepopulation){
		...
		if (parseInt(this.lastMarkedDayObject.getAttribute("isToday")) == 1){
			this.lastMarkedDayObject.setAttribute("group","dayNormalToday");
			this.lastMarkedDayObject.style.background = "url(img/" + currentColorScheme + "_dayNormal.gif) left top no-repeat";
		} else {
			this.lastMarkedDayObject.setAttribute("group","dayNormal");
			this.lastMarkedDayObject.style.background = "url(img/" + currentColorScheme + "_dayNormal.gif) left top no-repeat";
		}
	}
	...
	if (parseInt(anElement.getAttribute("isToday")) == 1){
		anElement.setAttribute("group","dayDownToday");
		anElement.style.background = "url(img/" + currentColorScheme + "_dayDown.gif) left top no-repeat";
	} else {
		anElement.setAttribute("group","dayDown");
		anElement.style.background = "url(img/" + currentColorScheme + "_dayDown.gif) left top no-repeat";
	}
};
...
JsDatePick.prototype.getDOMControlBar = function(){
	...
	var controlsBar 			= d.createElement("div");
	var monthForwardButton		= d.createElement("div");
	var monthBackwardButton		= d.createElement("div");
	var yearForwardButton		= d.createElement("div");
	var yearBackwardButton		= d.createElement("div");
	var controlsBarText			= d.createElement("div");
	
	controlsBar.setAttribute("group","controlsBar");
	monthForwardButton.setAttribute("group","monthForwardButton");
	monthBackwardButton.setAttribute("group","monthBackwardButton");
	yearForwardButton.setAttribute("group","yearForwardButton");
	yearBackwardButton.setAttribute("group","yearBackwardButton");
	controlsBarText.setAttribute("group","controlsBarText");
	...
	monthForwardButton.onmouseover = function(){
		...
		this.setAttribute("group","monthForwardButtonOver");
		var parentElement = this.parentNode;
		
		while (parentElement.groupName != "controlsBar"){
			parentElement = parentElement.parentNode;
		}				
		
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a month forward\");");
	};
	...
	monthForwardButton.onmouseout = function(){
		...
		this.setAttribute("group","monthForwardButton");
		var parentElement = this.parentNode;
	...
	monthForwardButton.onmousedown = function(){
		...
		this.setAttribute("group","monthForwardButtonDown");
		var parentElement = this.parentNode;
	...
	monthForwardButton.onmouseup = function(){
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a month forward\");");
		eval(globalRef + ".moveForwardOneMonth();");
	};
	...
	monthBackwardButton.onmouseover = function(){
		...
		this.setAttribute("group","monthBackwardButtonOver");
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a month backward\");");
	};
	...
	monthBackwardButton.onmouseout = function(){
		...
		this.setAttribute("group","monthBackwardButton");
		var parentElement = this.parentNode;
	...
	monthBackwardButton.onmousedown = function(){
		...
		this.setAttribute("group","monthBackwardButtonDown");
		var parentElement = this.parentNode;
	...
	monthBackwardButton.onmouseup = function(){
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a month backward\");");
		eval(globalRef + ".moveBackOneMonth();");		
	};
	...
	yearForwardButton.onmouseover = function(){
		...
		this.setAttribute("group","yearForwardButtonOver");
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a year forward\");");
	};
	...
	yearForwardButton.onmouseout = function(){
		...
		this.setAttribute("group","yearForwardButton");
		var parentElement = this.parentNode;
	...
	yearForwardButton.onmousedown = function(){
		...
		this.setAttribute("group","yearForwardButtonDown");
		var parentElement = this.parentNode;
	...
	yearForwardButton.onmouseup = function(){
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a year forward\");");
		eval(globalRef + ".moveForwardOneYear();");
	};
	...
	yearBackwardButton.onmouseover = function(){
		...
		this.setAttribute("group","yearBackwardButtonOver");
		var parentElement = this.parentNode;
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a year backward\");");
	};
	...
	yearBackwardButton.onmouseout = function(){
		...
		this.setAttribute("group","yearBackwardButton");
		var parentElement = this.parentNode;
	...
	yearBackwardButton.onmousedown = function(){
		...
		this.setAttribute("group","yearBackwardButtonDown");
		var parentElement = this.parentNode;
	...
	yearBackwardButton.onmouseup = function(){
		...
		var globalRef = "gRef_"+parentElement.getAttribute("globalNumber");
		eval(globalRef + ".setTooltipText(\"Move a year backward\");");
		eval(globalRef + ".moveBackOneYear();");
	};
